...
from interfaces_pkg.msg import ErpCmdMsg1, ErpStatusMsg1  # ← 통일
...
class CommandPublisher(Node):
    def __init__(self):
        ...
        # 상태 변수
        self.current_position = [0.0, 0.0]  # 시뮬 기준 원점 고정
    ...
    def odom_callback(self, msg: Odometry):
      ...
      # yaw 계산 (quaternion -> yaw)
      q = msg.pose.pose.orientation  # geometry_msgs/Quaternion (x,y,z,w)
      siny_cosp = 2.0 * (q.w * q.z + q.x * q.y)
      cosy_cosp = 1.0 - 2.0 * (q.y * q.y + q.z * q.z)
      self.current_yaw = math.atan2(siny_cosp, cosy_cosp)  # 라디안

      # 속도(필요하면)
      self.current_speed = msg.twist.twist.linear.x  # 보통 m/s
    ...
    def follow_waypoints(self):
        ...
        angle_to_wp = math.atan2(dy, dx)           # world-frame angle to waypoint
        ...
        # angle_error = (angle_to_waypoint + math.pi) % (2 * math.pi) - math.pi  # [-pi, pi]

        # 간단한 pure-pursuit 형태 조향
        wheel_base = 1.04
        ...
        # 최종 조향 명령: ±30도 → ±2000 카운트 (부호 반전 없음: 우<0, 좌>0 규약 가정)
        ...
        self.lat_err.append(abs(lateral_error))
        ...
        self.get_logger().info(
            f"[ERP42 CMD] Speed: {speed}, Steer: {steering_angle_cmd}, Curvature: {k:.4f}, LatErr(avg): {avg_lat_err:.2f}"
        )
    ...
    class PID:
        ...
        def control(self, error):
            ...
            self.node.get_logger().info(f"P: {self.Pterm:.2f}, I: {self.ki * self.Iterm:.2f}, D: {self.Dterm:.2f}")
            return output
